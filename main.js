...
(function() {
  ...
  const UI_section = document.querySelector('.svgpaint-container'),
        ...
        UI_restrictedColors = UI_section.querySelector('.restricted-colors .color-list'),
  ...
  let = colorsList = [],
        ...
        restrictedColorsListArr = ["#CAE4ED", "#252525", "#343434", "#8F8F8F", "#c4c4c4", "#e9f2f4", "#000000"];
  ...
  const app = {
    ...
    downloadSVGBtn: function(svg) {
      ...
      clone.querySelectorAll("*").forEach(el => {
        ...
        if (style.filter && style.filter !== "none") el.setAttribute("filter", style.filter);
      });
    ...
    filterMetallic: function(svg) {
      UI_filterMetallic.addEventListener('change', function(e) {
        ...
        // Control metallic by setting alpha to 0 (invisible) or 1 (visible)
        document.querySelector("#mf-metal")
          .setAttribute("slope", e.target.checked ? "1" : "0");
    ...
    filterUpdateColorTuning: function(svg) {
      [UI_filterSaturation, UI_filterBrightness, UI_filterContrast].forEach(item => {
        item.addEventListener('input', function() {
          ...
          svg.querySelectorAll("#mf-contrast feFuncR, #mf-contrast feFuncG, #mf-contrast feFuncB")
            .forEach(fn => {
    ...
    filterShineFun: function(svg) {
      UI_filterShine.addEventListener('change', function(e) {
        ...
        document.querySelector("#mf-shine-flood")
    ...
    injectMasterFilter: function(svg) {
      ...
      defs.innerHTML = `
        <filter id="master-filter" color-interpolation-filters="sRGB">
          ...
          <!-- METALLIC -->
          <feTurbulence type="fractalNoise" baseFrequency="1.2" numOctaves="3" seed="1" result="metalNoise"/>
          <feColorMatrix in="metalNoise" type="saturate" values="0" result="metalGray"/>
          <feComponentTransfer in="metalGray" result="metalContrast">
          ...
          <feComposite in="metalContrast" in2="afterShine" operator="in" result="metalMask"/>
          <feBlend mode="soft-light" in="metalMask" in2="afterShine" result="afterMetal"/>

          <!-- DROP SHADOW -->
          <feDropShadow id="mf-shadow" in="afterMetal" dx="0" dy="4"
            stdDeviation="6" flood-opacity="0" result="afterShadow"/>
